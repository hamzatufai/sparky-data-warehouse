...
-- a virtual warehouse, a database, RAW/ANALYTICS schemas, a custom role,
...
SELECT CURRENT_USER();
GRANT ROLE SPARKIFY_ROLE TO USER <USERNAME>;
...
-- Create the ANALYTICS schema: dbt writes the final star schema here.
CREATE SCHEMA IF NOT EXISTS SPARKIFY_DB.ANALYTICS
  COMMENT = 'Curated star schema (facts + dimensions) built by dbt';
...
-- Allow the role to see and use the database and both schemas.
GRANT USAGE ON DATABASE SPARKIFY_DB TO ROLE SPARKIFY_ROLE;
GRANT USAGE ON SCHEMA SPARKIFY_DB.RAW TO ROLE SPARKIFY_ROLE;
GRANT USAGE ON SCHEMA SPARKIFY_DB.ANALYTICS TO ROLE SPARKIFY_ROLE;
...
-- Allow the role (via dbt) to create views/tables for the star schema.
GRANT CREATE TABLE, CREATE VIEW
  ON SCHEMA SPARKIFY_DB.ANALYTICS TO ROLE SPARKIFY_ROLE;

-- Grant SELECT on all current and FUTURE tables in both schemas so dbt
...
GRANT SELECT ON ALL TABLES IN SCHEMA SPARKIFY_DB.ANALYTICS TO ROLE SPARKIFY_ROLE;
GRANT SELECT ON FUTURE TABLES IN SCHEMA SPARKIFY_DB.ANALYTICS TO ROLE SPARKIFY_ROLE;
